...
export function RastrearEncomiendaView() {
  // Prellenar y buscar de una desde "Mis encomiendas" (?guia=ECH-000123),
  // sin exponer datos personales en la URL: solo el número de guía, que no
  // es un dato personal bajo Ley 29733.
  const guiaInicial = useSearchParams().get("guia") ?? "";

  const [guia, setGuia] = useState(guiaInicial);
  // Si llega ?guia= desde "Mis encomiendas", arranca directo en "cargando":
  // el efecto de abajo dispara la búsqueda una sola vez, al montar.
  const [estado, setEstado] = useState<Estado>(guiaInicial ? "cargando" : "inicial");
  const [resultado, setResultado] = useState<Encomienda | null>(null);

  useEffect(() => {
    if (!guiaInicial) return;
    const timeoutId = setTimeout(() => {
      const encontrada = buscarEncomiendaPorGuia(guiaInicial);
      if (!encontrada) {
        setResultado(null);
    ...
    // Solo al montar: si la persona edita el campo después, no debe
    // volver a dispararse por el valor inicial de la URL.
