...
export interface MisComprasViewProps {
  forzarError: boolean;
}
...
export function MisComprasView({ forzarError }: MisComprasViewProps) {
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-6 sm:py-8">
      <h1 className="text-xl font-semibold text-navy sm:text-2xl">Mis Viajes</h1>
      <p className="mt-1 text-sm text-navy/70">Tus pasajes comprados y su estado.</p>

      <div className="mt-4 flex flex-col gap-4">
        {estado === "cargando" && (
          <div role="status" aria-label="Cargando tus compras" className="flex flex-col gap-4">
        ...
        {estado === "listo" && MIS_COMPRAS.length === 0 && (
          <EmptyState
            icon={<TicketIcon />}
